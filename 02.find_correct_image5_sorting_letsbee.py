...
# # 1. DATA
# np.load
x_train = np.load('../Project01_data/9.npy/color_x_train_letsbee.npy')
x_test = np.load('../Project01_data/9.npy/color_x_test_letsbee.npy')
x_valid = np.load('../Project01_data/9.npy/color_x_valid_letsbee.npy')
y_train = np.load('../Project01_data/9.npy/color_y_train_letsbee.npy')
y_test = np.load('../Project01_data/9.npy/color_y_test_letsbee.npy')
y_valid = np.load('../Project01_data/9.npy/color_y_valid_letsbee.npy')
...
model = load_model('../Project01_data/9.cp/find_letsbee_0.0068.hdf5')
...
def prepro_n_predict (whole_list) : 
    ...
    for img in whole_list :
        try :
            ...
            # 0이면 다른 폴더에 사진 복사 & 저장
            if pred == 0 :
                copy_path = '../Project01_data/sorting_correct/letsbee/'+str(n)+'.jpg'
...
print("전체 데이터 개수 ",len(letsbee_list))
result = prepro_n_predict(letsbee_list)
# print(result)
print(">>> copy end >>>")
