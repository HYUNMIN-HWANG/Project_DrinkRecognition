...
# # 1. DATA
# np.load
x_train = np.load('../Project01_data/9.npy/color_x_train_tejava.npy')
x_test = np.load('../Project01_data/9.npy/color_x_test_tejava.npy')
x_valid = np.load('../Project01_data/9.npy/color_x_valid_tejava.npy')
y_train = np.load('../Project01_data/9.npy/color_y_train_tejava.npy')
y_test = np.load('../Project01_data/9.npy/color_y_test_tejava.npy')
y_valid = np.load('../Project01_data/9.npy/color_y_valid_tejava.npy')
...
def prepro_n_predict (whole_list) : 
    ...
    for img in whole_list :
        try :
            ...
            # 0이면 다른 폴더에 사진 복사 & 저장
            if pred == 0 :
                copy_path = '../Project01_data/sorting_correct/tejava/'+str(n)+'.jpg'
...
print("전체 데이터 개수 ",len(tejava_list))
result = prepro_n_predict(tejava_list)
# print(result)
print(">>> copy end >>>")
